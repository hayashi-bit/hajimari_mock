...
支払期限：2026年2月10日(火) 23:59""",
    ...
    {
        # 2026-06-21 A方式の網に混入する宣伝の代表例①（ぴあ・おまかせエントリー告知）。
        # 件名に「ずっと真夜中」「抽選発売」が入り網に引っかかるが、まだ申込/購入していない＝false。
        # Gmailの「プロモーションを含めない」でも残るため、一次判定(classify)で弾く必要がある。
        "id": "pia_omakase_promo",
        "subject": "おまかせエントリー対象外の抽選発売のお知らせ [ずっと真夜中でいいのに。]",
        "from": "pia/おまかせエントリー <info@pia.jp>",
        "received_at": "2026-03-16T12:00:00+09:00",
        "body": """「おまかせエントリー」をご利用のお客様へ、対象外の抽選発売のお知らせです。
下記公演は「おまかせエントリー」の対象外のため、自動申込は行われません。
ご希望の場合はご自身でお申し込みください。
公演：ずっと真夜中でいいのに。
受付：先行抽選
※本メールは申込・購入を確定するものではありません。""",
        "expect": {
            "isTicketRelated": False,  # 宣伝・先行案内＝まだ申込/購入していない→除外
        },
    },
    {
        # A方式の網に混入する宣伝の代表例②（一般発売の告知）。購入実績ではない＝false。
        "id": "pia_onsale_promo",
        "subject": "OSAKA GIGANTIC MUSIC FESTIVAL 2026 一般発売のお知らせ",
        "from": "pia/お気に入り <info@pia.jp>",
        "received_at": "2026-05-30T10:00:00+09:00",
        "body": """お気に入り登録いただいた公演の一般発売が開始されました。
OSAKA GIGANTIC MUSIC FESTIVAL 2026
一般発売：本日10:00より先着受付中。お早めにお求めください。
※本メールは販売告知です。購入を確定するものではありません。""",
        "expect": {
            "isTicketRelated": False,  # 販売告知＝除外
        },
    },
    {
