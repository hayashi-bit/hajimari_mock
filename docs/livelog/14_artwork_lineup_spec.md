...
## カバー率テスト結果（2026-06-18・実ユーザーの実リストで検証）
実際に行ったライブのアーティストから**インディー中心に10組をSpotify在籍確認 → 全組ヒット**。
前髪ぱっつん少年／紫今／RuLu／NOMELON NOLEMON／水槽／Aooo／はしメロ／CLAN QUEEN／長瀬有花／**アザミ（月間5.1千人でも在籍）**。
ほか ずとまよ/ロクデナシ/meiyo/ReoNa/美波/かいりきベア/MAISONdes/ぼっちぼろまる/チョーキューメイ も確実。
→ **結論：このセグメントは Spotify でほぼ100%カバーできる。Apple Music は現状不要**（データで穴が見えたら後付け）。
→ ごく稀の「在籍だが画像なし／真のローカル」は **ユーザー投稿**で補う。
...
- Spotify で not_found / needs_review / 画像なし の場合は、UIで「写真を追加」をユーザーに促す（imageSource='user'）。
  ※実データ検証ではSpotifyでほぼ100%取れたため、Apple Music等の追加プロバイダは"今は不要"。
  将来 not_found 率が上がったら、取得処理を差し替え可能なチェーン(Spotify→Apple/Deezer→user)にできるよう、プロバイダを関数で分離しておく。

【表示】
- ライブカード・アーティストプロフィール・(将来)フェス出演者一覧で imageUrl を表示。無い時はプレースホルダ＋「写真を追加」導線。
