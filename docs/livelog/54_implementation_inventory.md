...
| 4 | Heartbeat(Gmail増分同期) | 🟡Day4着手中 | STEP A完了：cron3本(gmail-incremental自動登録/auto-archive/daily-backup)は**発火・認証・CRUD全てForge依存**、CRON_SECRET/railway cron定義は未存在。方針確定=**A①Railwayネイティブcron/B毎時グローバル1回・全ユーザー反復/C CRON_SECRET(Bearer)/D移行中はForge isCronフォールバック残す**。**`CRON_SECRET`はRailway登録済**。STEP B実装=PR#8マージ済。**Railway cronサービス3本 立ち上げ完了(2026-07-01)**：cron-gmail(毎時`0 * * * *`)/cron-backup(日次`0 0 * * *`)/cron-archive(日次`0 15 * * *`)・各に APP_URL＋CRON_SECRET・Restart Policy=Never。スケジュール認識OK(Next in で確認)。残＝cron-gmailの実発火200確認→数日OKで②がForge cron撤去(最終PR) |
| 5 | LLM | 🟡Day5着手中 | STEP A完了：`invokeLLM`はOpenAI互換でForge/Gemini(gemini-2.5-flash)を叩く・呼び出し14か所(Vision5/構造化多数/json_object1/tools不使用)。方針=**返却契約(OpenAI形)不変で中身だけAnthropic Messages APIへ**・構造化はtool-use変換・VisionはURL image・**inert-until-key**(ANTHROPIC_API_KEY未投入なら現行Gemini)。モデル=**分類Haiku4.5/抽出・VisionSonnet5**(Opus不使用)。**ANTHROPIC_API_KEY Railway登録済(2026-07-01・組織クレジット$5)**。voiceTranscription(Whisper)は別スコープ。②がSTEP B実装中 |
