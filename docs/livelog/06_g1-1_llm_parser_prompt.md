...
- 推奨: **`claude-haiku-4-5`**。理由＝チケットメール抽出は短文・定型で、最安モデルで十分な精度が出る。1通あたり約0.0002ドル（≒0.03円）、月50通×100人でも月150円程度。
- 既存 `api/chat.ts` は `claude-sonnet-4-6` を使用中。精度が不足する場合のみ Sonnet 4.6 に上げる。
- 構造化出力（JSON Schema強制）は Haiku 4.5 / Sonnet 4.6 ともに対応。
